    ...
    inline fn getCpuCount() !usize {
        ...
        switch (builtin.os.tag) {
            ...
            .openbsd => {
                ...
                try posix.sysctl(&mib, &count, &count_size, null, 0);
                return @as(usize, @intCast(count));
            },
            .solaris, .illumos => {
                const rc = std.c.sysconf(std.c._SC.NPROCESSORS_ONLN);
            ...
            .haiku => {
                ...
                switch (posix.errno(rc)) {
                    .SUCCESS => return @as(usize, @intCast(system_info.cpu_count)),
                    else => |err| return posix.unexpectedErrno(err),
                }
            },
            .macos, .ios, .tvos, .watchos, .visionos => {
                ...
                try posix.sysctlbynameZ("hw.logicalcpu", &count, &count_len, null, 0);
                return @as(usize, @intCast(count));
            },
            .freebsd, .netbsd, .dragonfly => {
                var count: c_int = undefined;
                var count_len: usize = @sizeOf(c_int);
                try posix.sysctlbynameZ("hw.ncpu", &count, &count_len, null, 0);
                return @as(usize, @intCast(count));
            },
            .wasi => {
